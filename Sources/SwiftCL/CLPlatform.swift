...
public class CLPlatform {
	...
	public class var all: [CLPlatform] {
      ...
      let platforms: [CLPlatform] = platformIds.compactMap {
			guard let platformId = $0 else { return nil }
			return CLPlatform(id: platformId)
		}
		return platforms
	}
	...
	public func getInfo(_ info: cl_platform_info) -> String? {
		...
		return String(cString: &infoArray)
	}
	
	public func getDevices(_ deviceType: CLDeviceType = .gpu) -> [CLDevice] {
      ...
      let devices: [CLDevice] = deviceIds.compactMap {
			guard let deviceId = $0 else { return nil }
...
extension CLPlatform: CustomStringConvertible {
